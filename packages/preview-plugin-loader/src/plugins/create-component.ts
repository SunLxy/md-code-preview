...
import {
  markdownParsePlugin,
  getFileDirName,
  markdownParseCreateComponentPlugin,
} from "../utils";
...
class MdCreateComponentsPlugin {
  // 1. 输出目录，
  // 2. 哪些文件排除不进行监听
  // 3. 监听的根目录
  // 4. 去除的根路径
  ...
  readFile = (filePath: string) => {
    ...
    const result = markdownParseCreateComponentPlugin(mdStr, this.lang);
    if (result) {
      const dirPath = path.join(this.output, `${fileDirName}.js`);
      FS.writeFileSync(dirPath, result, { flag: "w+", encoding: "utf-8" });
    }
  };
...
export default MdCreateComponentsPlugin;
