...
export const markdownParse = (source: string, lang: string[]) => {
  ...
  markdownTree.children.map((itemChild) => {
    if (itemChild && itemChild.type === "code" && lang.includes(itemChild.lang)) {
...
// plugin 中转换
export const markdownParsePlugin = (source: string, fileDirName: string, savePath: string, lang: string[] = ["jsx", "tsx"]) => {
